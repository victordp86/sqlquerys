SELECT er.cliente_id AS "ID CLIENTE",s.descripcion  AS "GÉNERO", YEAR(DATE(NOW())) - YEAR(cl.fechaNacimiento) AS "EDAD", cl.codigoPostal AS "CP",cncc.descripcion AS "¿CÓMO NOS HA CONOCIDO?", i.descripcion AS "CENTRO", DATE(er.fecha) AS "FECHA ENCUESTA", 
  pr.textoRespuesta AS "RESPUESTA",
...
LEFT JOIN ComoNosConocioCliente cncc ON cncc.id = cl.comoNosConocioCliente_id 
LEFT JOIN Instalacion i ON cl.instalacion_id = i.id 
